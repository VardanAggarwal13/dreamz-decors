...
function FaqPage({ page }) {
  ...
  return (
    <div className="bg-bone">
      ...
      <div className="border-t border-hairline/60 py-8 sm:py-10 text-center">
        <div className="container-page">
          <MessageCircle size={26} strokeWidth={1.5} className="mx-auto text-gold/50" />
          <h2 className="mt-3 font-display text-xl text-ink sm:text-2xl">Still have questions?</h2>
          <p className="mt-1.5 text-xs sm:text-sm text-ink-soft">Our team is happy to help you personally.</p>
...
function AboutPage({ page }) {
  ...
  return (
    <div className="bg-bone">
      ...
      {/* ── 1. Centered Hero ──────────────────────────────────── */}
      <div className="border-b border-hairline/60 bg-bone py-8 text-center sm:py-10">
        <div className="container-page">
          <p className="eyebrow-gold">{page.eyebrow}</p>
          <h1 className="mx-auto mt-2.5 max-w-2xl whitespace-pre-line font-display text-3xl leading-tight text-ink sm:text-4xl lg:text-5xl">
            {page.title}
          </h1>
          <p className="mx-auto mt-3.5 max-w-lg text-xs sm:text-sm leading-relaxed text-ink-soft">
            {page.intro}
          </p>
        </div>
      </div>
      ...
      <div className="container-page py-8 sm:py-10 lg:py-12">
        <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-12">
          ...
          <div>
            ...
            <h2 className="mt-2.5 whitespace-pre-line font-display text-2xl leading-tight text-ink sm:text-3xl">
              {page.storyTitle}
            </h2>
            <div className="mt-4 space-y-3">
              {body.map((para, i) => (
                <p key={i} className="text-xs sm:text-sm leading-relaxed sm:leading-7 text-ink-soft">{para}</p>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="border-y border-hairline/60 bg-bone-soft/60 py-8 sm:py-10 lg:py-12">
        <div className="container-page">
          <div className="text-center">
            <p className="eyebrow-gold">{page.valuesEyebrow}</p>
            <h2 className="mt-2 font-display text-2xl text-ink sm:text-3xl">{page.valuesTitle}</h2>
            <span className="gold-rule-center" />
          </div>
          <div className="mt-7 sm:mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {values.map(({ icon, title, text }) => {
              const Icon = ABOUT_ICONS[icon] || ShieldCheck;
              return (
                <div key={title} className="rounded-2xl border border-hairline/60 bg-bone p-5 text-center">
                  <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full border border-hairline/60">
                    <Icon size={17} strokeWidth={1.5} className="text-gold" />
                  </div>
                  <h3 className="mt-3.5 text-sm font-semibold text-ink">{title}</h3>
                  <p className="mt-1.5 text-xs sm:text-sm leading-relaxed text-ink-soft">{text}</p>
                </div>
              );
            })}
          </div>
      ...
      <div className="bg-ink py-10 text-center sm:py-12 lg:py-14">
        <div className="container-page">
          <p className="text-[10px] uppercase tracking-[0.3em] text-gold/60">{page.craftEyebrow}</p>
          <h2 className="mx-auto mt-3 max-w-2xl whitespace-pre-line font-display text-2xl leading-snug text-bone-soft sm:text-3xl lg:text-4xl">
            {page.craftTitle}
          </h2>
          <p className="mx-auto mt-3.5 max-w-xl text-xs sm:text-sm leading-relaxed sm:leading-7 text-bone/55">
            {page.craftText}
          </p>
          <Button
            asChild
            variant="outline"
            size="md"
            className="mt-6 border-bone/35 text-bone hover:border-bone/60 hover:bg-bone/10"
          >
      ...
      <div className="py-8 sm:py-10 lg:py-12">
        <div className="container-page">
          <div className="text-center">
            <h2 className="font-display text-2xl text-ink sm:text-3xl">{page.statsTitle}</h2>
            <span className="gold-rule-center" />
          </div>
          <div className="mt-7 sm:mt-8 grid grid-cols-2 gap-3.5 lg:grid-cols-4">
            {stats.map(({ value, label }) => (
              <div key={label} className="rounded-2xl border border-hairline/60 p-5 text-center">
                <p className="font-display text-2xl text-gold sm:text-3xl">{value}</p>
                <p className="mt-1.5 text-[9px] uppercase tracking-[0.22em] text-ink-muted sm:text-[10px]">{label}</p>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="bg-bone">
      ...
      <div className="border-b border-hairline/60 bg-bone py-7 text-center sm:py-9">
        <div className="container-page">
          ...
          <span className="gold-rule-center" />
          <p className="mx-auto mt-2.5 max-w-md text-xs sm:text-sm leading-relaxed text-ink-soft">{page.intro}</p>
        </div>
      </div>
      ...
      <div className="container-page py-8 sm:py-10">
        <div className="grid gap-6 lg:grid-cols-[3fr_2fr] lg:gap-8">
          ...
          {/* Form — left */}
          <div>
            <h2 className="font-display text-lg sm:text-xl text-ink">Send Us a Message</h2>
            <form onSubmit={submit} className="mt-4 space-y-3.5">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="mt-1.5 w-full rounded-xl border border-hairline bg-bone-soft px-4 py-2.5 text-sm text-ink outline-none transition placeholder:text-ink-muted focus:border-accent/60"
                />
              </div>
          ...
          {/* Details — right */}
          <div>
            <h2 className="font-display text-lg sm:text-xl text-ink">Our Details</h2>
            <div className="mt-4 space-y-2">
              {cards.map(({ Icon, label, value, href }) => (
                <div
                  key={label}
                  className="flex items-start gap-3 rounded-xl border border-hairline/60 bg-bone-soft p-3.5 transition hover:border-gold/40"
                >
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-hairline/60">
                    <Icon size={14} className="text-gold" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[12px] font-semibold text-ink">{label}</p>
                    {href ? (
                      <a href={href} className="mt-0.5 block break-words text-xs sm:text-sm text-ink-soft transition hover:text-accent">
                        {value}
                      </a>
                    ) : (
                      <p className="mt-0.5 whitespace-pre-line break-words text-xs sm:text-sm text-ink-soft">{value}</p>
                    )}
                  </div>
                </div>
              ))}
              ...
              {/* Response nudge */}
              <div className="rounded-xl border border-hairline/60 bg-bone p-3.5">
                <p className="text-[12px] font-semibold text-ink">Response Time</p>
      ...
      {/* ── Find Us / Map ─────────────────────────────────────── */}
      <div className="border-t border-hairline/60 py-8 sm:py-10">
        <div className="container-page">
          <div className="text-center">
            <p className="eyebrow-gold">Location</p>
            <h2 className="mt-2 font-display text-2xl text-ink sm:text-3xl">Find Us</h2>
            <span className="gold-rule-center" />
          </div>
          <div className="mt-6 overflow-hidden rounded-2xl border border-hairline/60 shadow-sm">
            <iframe
    ...
    <div className="bg-bone">
      ...
      <div className="border-b border-hairline/60 bg-bone py-7 text-center sm:py-9">
        <div className="container-page">
          ...
          <span className="gold-rule-center" />
          <p className="mx-auto mt-2.5 max-w-md text-xs sm:text-sm leading-relaxed text-ink-soft">{page.intro}</p>
          <div className="mx-auto mt-5 flex flex-wrap justify-center gap-6 border-t border-hairline/60 pt-4 sm:gap-8">
            {page.heroStats.map(stat => (
              <div key={stat.label} className="text-center">
                <p className="font-display text-base text-gold sm:text-lg">{stat.value}</p>
                <p className="mt-0.5 text-[9px] uppercase tracking-[0.22em] text-ink-muted sm:text-[10px]">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="border-b border-hairline/60 bg-bone-soft/50 py-8 sm:py-10 lg:py-12">
        <div className="container-page">
          <div className="text-center">
            <p className="eyebrow-gold">How It Works</p>
            <h2 className="mt-2 font-display text-2xl text-ink sm:text-3xl">Your Order's Journey</h2>
            <span className="gold-rule-center" />
          </div>
          <div className="mt-7 sm:mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {steps.map(({ title, text }, i) => (
              <div key={title} className="rounded-xl border border-hairline/60 bg-bone p-4 sm:p-5">
                <span className="font-display text-xl text-gold/40">0{i + 1}</span>
                <h3 className="mt-2.5 text-xs sm:text-sm font-semibold uppercase tracking-[0.12em] text-ink">{title}</h3>
                <p className="mt-1.5 text-xs sm:text-sm leading-relaxed text-ink-soft">{text}</p>
              </div>
            ))}
          </div>
      ...
      <div className="border-b border-hairline/60 py-8 sm:py-10 lg:py-12">
        <div className="container-page">
          <div className="text-center">
            <p className="eyebrow-gold">Timelines</p>
            <h2 className="mt-2 font-display text-2xl text-ink sm:text-3xl">Estimated Delivery</h2>
            <span className="gold-rule-center" />
          </div>
          <div className="mt-7 sm:mt-8 grid gap-4 sm:grid-cols-3">
            {deliveryTimes.cards.map(card => (
              <div key={card.title} className="rounded-xl border border-hairline/60 bg-bone-soft p-5">
                <p className="text-[13px] font-semibold text-ink">{card.title}</p>
                <p className="mt-1.5 text-xs sm:text-sm leading-relaxed text-ink-soft">{card.text}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-3xl space-y-6">
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1">
              <h2 className="font-display text-lg font-semibold text-ink sm:text-xl">Shipping Policy</h2>
              <ul className="mt-3 space-y-2">
                {info.bullets.map((item, i) => (
                  <li key={i} className="flex items-start gap-2 text-xs sm:text-sm leading-relaxed sm:leading-7 text-ink-soft">
                    <ChevronRight size={14} className="mt-[3px] shrink-0 text-gold" />
                    {item}
                  </li>
                ))}
              </ul>
          ...
          <div className="flex gap-4">
            <div className="mt-1 w-0.5 shrink-0 self-stretch rounded-full bg-gold/50" />
            <div className="flex-1">
              <h2 className="font-display text-lg font-semibold text-ink sm:text-xl">Delivery Charges</h2>
          ...
          <div className="flex gap-4">
            <div className="mt-1 w-0.5 shrink-0 self-stretch rounded-full bg-gold/50" />
            <div className="flex-1">
              <h2 className="font-display text-lg font-semibold text-ink sm:text-xl">Order Tracking</h2>
              <div className="mt-3 space-y-2">
      ...
      {/* ── 5. Help CTA ───────────────────────────────────────── */}
      <div className="border-t border-hairline/60 py-8 text-center">
        <div className="container-page">
          <p className="text-xs sm:text-sm font-medium text-ink">Have a question about your delivery?</p>
          <p className="mt-1 text-xs sm:text-sm text-ink-soft">Our team replies within 1 business day.</p>
          <Button asChild variant="primary" size="md" className="mt-4 uppercase tracking-[0.14em]">
            <Link to="/contact">Contact Us <FiArrowRight /></Link>
          </Button>
        </div>
      </div>
...
function TermsPage({ page }) {
  return (
    <div className="bg-bone">
      ...
      {/* ── Hero — left-aligned ───────────────────────────────── */}
      <div className="border-b border-hairline/60 bg-bone py-7 sm:py-9">
        <div className="container-page">
          <p className="eyebrow-gold">Legal</p>
          <h1 className="mt-2 font-display text-2xl sm:text-3xl lg:text-4xl leading-tight text-ink">
            Terms &amp; Conditions
          </h1>
          <p className="mt-2 text-xs sm:text-sm text-ink-muted">{page.intro}</p>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-3xl space-y-6">
          {page.sections.map((section, i) => (
            <section key={section.title} className="flex gap-4">
              ...
              <div className="flex-1">
                <h2 className="font-display text-[15px] font-semibold text-ink sm:text-[17px]">
                  {i + 1}. {section.title}
                </h2>
                ...
                {section.body && (
                  <div className="mt-2.5 space-y-2">
                    {section.body.map((para, pi) => (
                      <p key={pi} className="text-xs sm:text-sm leading-relaxed sm:leading-7 text-ink-soft">{para}</p>
                    ))}
                  </div>
                )}
                ...
                {section.bullets && (
                  <ul className="mt-2.5 space-y-1.5">
                    {section.bullets.map((item, bi) => (
                      <li key={bi} className="flex items-start gap-2 text-xs sm:text-sm leading-relaxed sm:leading-7 text-ink-soft">
                        <ChevronRight size={14} className="mt-[3px] shrink-0 text-gold" />
                        {item}
                      </li>
                    ))}
